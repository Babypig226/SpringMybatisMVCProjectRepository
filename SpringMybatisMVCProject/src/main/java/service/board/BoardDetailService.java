...
public class BoardDetailService {
	...
	public String boardDelete(Integer boardNum, String boardPass, HttpSession session, Model model) {
		AuthInfo authInfo = (AuthInfo)session.getAttribute("authInfo");
		BoardDTO dto = new BoardDTO();
		dto.setBoardNum(boardNum);
		dto.setUserId(authInfo.getUserId());
		BoardDTO board = boardRepository.getBoardList(dto).get(0);
		
		if(bcryptPasswordEncoder.matches(boardPass, board.getBoardPass())) {
			dto.setBoardPass(boardPass);
			int i = boardRepository.boardDelete(board);
			model.addAttribute("val", i);
			return "board/delPage";
		}else {
			model.addAttribute("badPass", "비밀번호가 잘못되었습니다.");
			return "board/qna_board_modify";
		}
